/**
 * eval_02.ts — Paris briefing → Claude API → structured JSON validation
 *
 * Usage: npm run eval:02
 * Requires: ANTHROPIC_API_KEY env var
 *
 * Results are written to: evals/results/eval_02_result.json
 */
...
const evalData = JSON.parse(readFileSync(resolve(root, 'evals/eval_02_paris.json'), 'utf-8'))
...
async function runEval() {
  ...
  if (Array.isArray(itin['days'])) {
    ...
    check(days.length === 5, `Expected 5 days, got ${days.length}`)
  ...
  if ('traveler_profile' in itin) {
    const profile = itin['traveler_profile'] as Record<string, unknown>
    const restrictions = profile['restrictions'] as string[] | undefined
    const hasVegetarian = restrictions?.some(r =>
      r.toLowerCase().includes('vegetarian') || r.toLowerCase().includes('vegeta'),
    )
    check(!!hasVegetarian, 'traveler_profile.restrictions: vegetarian constraint not captured')
  }
  ...
  if ('trip_metadata' in itin) {
    ...
    if (meta['companion_type'] === evalData.expected_companion_type) {
      ...
      console.log(`~ Companion type mismatch: expected ${evalData.expected_companion_type}, got ${meta['companion_type']}`)
    }

    if (meta['budget_tier'] === evalData.expected_budget_tier) {
      console.log(`✓ Budget tier correctly detected: ${meta['budget_tier']}`)
    } else {
      console.log(`~ Budget tier mismatch: expected ${evalData.expected_budget_tier}, got ${meta['budget_tier']}`)
    }
  }
  ...
  const result = {
    ...
    validation: {
      passed,
      errors,
      persona_match: meta['detected_persona'] === evalData.expected_persona,
      companion_match: meta['companion_type'] === evalData.expected_companion_type,
      budget_match: meta['budget_tier'] === evalData.expected_budget_tier,
      vegetarian_captured: (profile['restrictions'] as string[] | undefined)?.some(r =>
        r.toLowerCase().includes('vegetarian') || r.toLowerCase().includes('vegeta'),
      ) ?? false,
    },
    itinerary,
  ...
  const outPath = resolve(resultsDir, 'eval_02_result.json')
  writeFileSync(outPath, JSON.stringify(result, null, 2))
  console.log(`\n→ Results saved to evals/results/eval_02_result.json`)
